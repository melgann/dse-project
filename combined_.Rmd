...
```{r}
# Calculate the number of bus stops for each streets and 
number_of_bus_stops_near_street <- results_df %>%
  group_by(StreetName) %>%
  summarise(number_of_bus_stops=n(), dist_to_nearest_bus_stop = min(Distance))
```
...
```{r}
df_scaled <- final_df %>%
  dplyr::select(-c("Street_name", "Planning_Area.x", "Planning_Area.y", "X", "longitude", "latitude")) %>%
